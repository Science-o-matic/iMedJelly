				...
				if(section.equals("beachComments") && reader.peek() != JsonToken.NULL) {
					...
					while(reader.hasNext()) {
						...
						while (reader.hasNext()) {
							...
							else if (item.equals("response") && reader.peek() != JsonToken.NULL){
								// Add the CSIC response.
						...
						if (response != null) {
							comments.add(new Comment("CSIC", response, format, null));
						}
						reader.endObject();
					}
					reader.endArray();
	...
	public void parseJson(JsonReader reader) throws IOException{
		...
		while (reader.hasNext()) {
			...
			if (item.equals("avgRating") && reader.peek() != JsonToken.NULL) {
				...
				flagStatusUpdated = reader.nextString();
			}
